...
export default async function DictionarySearchPage({ searchParams }: { searchParams: { q?: string } }) {
  const q = searchParams.q || "";
  let results: InferSelectModel<typeof wordsTable>[] = [];
  if (q.length > 0) {
    results = await db.select().from(wordsTable).where(like(wordsTable.word, `%${q}%`));
  }
  return (
    <div className="max-w-2xl mx-auto p-8">
      ...
      {q.length === 0 ? (
        ...
        <div className="text-red-500">No results found for &quot;{q}&quot;.</div>
      ) : (
